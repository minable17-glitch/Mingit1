...
- 단계별 마감(YYYY-MM-DD)은 오늘 날짜와 업무 최종 마감 사이에 무리 없이 배분합니다. 최종 마감이 없으면 합리적으로 추정하거나 비워 둡니다(빈 문자열).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (e instanceof Anthropic.APIError) return json({ error: `api_${e.status}` }, 502);
