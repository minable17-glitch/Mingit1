-- 업무 챙김 한 번에 설치 (schema.sql + schema_phase2.sql + schema_public.sql + 관리자 이메일 등록)
-- 업무 챙김용 새 Supabase 프로젝트의 SQL Editor에 전체를 붙여넣고 Run 한 번. 여러 번 실행해도 안전합니다.
-- ※ schema.sql / schema_phase2.sql / schema_public.sql 을 고치면 이 파일도 같이 고쳐야 합니다.
...
-- 관리자 이메일 등록 (관리자는 AI·고급 구글 연동을 쓰고, 가입 열기/닫기와 사용자별 기능을 관리)
